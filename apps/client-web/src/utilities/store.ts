...
export const loadInitialData = async (params: {
  ...
  try {
    ...
    if (!clientItems?.length) {
      ...
      const filteredClientItems = clientItems.filter(
        (item) => item.sync_status != SyncStatus.DELETED
      );
      ...
      if (!serverItems.length) {
        mergedItems = filteredClientItems;
        lengthComparison = mergedItems.length !== filteredClientItems.length;
        serialComparison =
          JSON.stringify(mergedItems) !== JSON.stringify(filteredClientItems);
      } else {
